    ...
    <div className="py-24" id="integration-section">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-20">
        <div className="space-y-12">
          ...
          <button className="border-2 border-white text-white font-semibold px-8 py-3 rounded-xl hover:bg-white/10 transition-all shadow-xl mt-8">
            Get Started
          </button>
        </div>
            ...
            {categories.map((cat) => (
              <button
                ...
                className={`pb-4 px-6 text-sm font-bold transition-all relative ${
                  activeCat === cat.id ? 'text-white' : 'text-white/40'
                }`}
              >
                {cat.label}
                {activeCat === cat.id && (
